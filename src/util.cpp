...
const char * const BITCOIN_CONF_FILENAME = "anomaly.conf";
const char * const BITCOIN_PID_FILENAME = "anomalyd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Anomaly
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Anomaly
    // Mac: ~/Library/Application Support/Anomaly
    // Unix: ~/.anomaly
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Anomaly";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Anomaly";
#else
    // Unix
    return pathRet / ".anomaly";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Anomlay Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Anomlay Core developers";
    }
