...
- [Building Qtum Core](#building-anomaly-core)
...
Clone the git repositories for anomaly and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/anomalyproject/anomaly --recursive
git clone https://github.com/anomalyproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/anomaly/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the anomaly repository under 'Fetch and create inputs' to install sources which require
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/anomaly/.git/
...
For example:
```bash
URL=https://github.com/laanwj/anomaly.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit anomaly=${COMMIT} --url anomaly=${URL} ../anomaly/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit anomaly=${COMMIT} --url anomaly=${URL} ../anomaly/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit anomaly=${COMMIT} --url anomaly=${URL} ../anomaly/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-anomaly:
./bin/gbuild --commit anomaly=${COMMIT},cpp-eth-anomaly=${ETHCOMMIT} --url anomaly=${URL},cpp-eth-anomaly=${ETHURL} ../anomaly/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the anomaly git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../anomaly/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/anomalyproject/anomaly-detached-sigs.git

BTCPATH=/some/root/path/anomaly
SIGPATH=/some/root/path/anomaly-detached-sigs

./bin/gbuild --url anomaly=${BTCPATH},signature=${SIGPATH} ../anomaly/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/anomaly-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/anomaly-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/anomaly-osx-build.assert
```
...
[anomalyproject/gitian.sigs](https://github.com/anomalyproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@anomaly.org) and he will commit them.